...
## [your code here]
arrange(filter(df[,c("X","Grad.Rate")], Grad.Rate >= 80),desc(Grad.Rate))
...
## [your code here]
summarise(group_by(df,Private), max.tuition=max(Outstate),min.tuition=min(Outstate))

summarise(group_by(college,Private), max.tuition=max(Outstate),min.tuition=min(Outstate))
